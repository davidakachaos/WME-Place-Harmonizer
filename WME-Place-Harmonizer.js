/* eslint-disable no-prototype-builtins, nonblock-statement-body-position, brace-style, curly, radix, no-template-curly-in-string */
// ==UserScript==
...
const _WHATS_NEW_LIST = {
  ...
  "2020.09.19.001": ["Updated place service buttons title."],
  "2020.09.07.001": [
  ...
  "2020.06.02.001": ["Bug fix due to latest WME release."],
};
...
let _pnhLockLevel;
const _PM_USER_LIST = {
  // user names and IDs for PM functions
  ...
  WMEPH: {
    approvalActive: true,
    mods: [{ id: "2647925", name: "MapOMatic" }],
  },
};
...
const _UPDATED_FIELDS = {
  name: {
    updated: false,
    selector: '.venue .form-control[name="name"]',
    tab: "general",
  },
  aliases: {
    updated: false,
    selector: ".venue .form-control.alias-name",
    tab: "general",
  },
  address: {
    updated: false,
    selector: ".venue .address-edit span.full-address",
    tab: "general",
  },
  categories: {
    updated: false,
    selector: ".venue .categories.controls .select2-container",
    tab: "general",
  },
  description: {
    updated: false,
    selector: '.venue .form-control[name="description"]',
    tab: "general",
  },
  lock: {
    updated: false,
    selector: ".venue .form-control.waze-radio-container",
    tab: "general",
  },
  externalProvider: {
    updated: false,
    selector: ".venue .external-providers-view",
    tab: "general",
  },
  brand: {
    updated: false,
  ...
  url: {
    updated: false,
    selector: '.venue .form-control[name="url"]',
    tab: "more-info",
  },
  phone: {
    updated: false,
    selector: '.venue .form-control[name="phone"]',
    tab: "more-info",
  },
  openingHours: {
    updated: false,
    selector: ".venue .opening-hours ul",
    tab: "more-info",
  },
  cost: {
    updated: false,
    selector: '.venue .form-control[name="costType"]',
    tab: "more-info",
  },
  canExit: {
    updated: false,
  ...
  lotType: {
    updated: false,
    selector: ".venue .parking-type-option",
    tab: "more-info",
  },
  parkingSpots: {
    updated: false,
    selector: '.venue .form-control[name="estimatedNumberOfSpots"]',
    tab: "more-info",
  },
  lotElevation: {
    updated: false,
  ...
  updateEditPanelHighlights() {
    ...
    this.getFieldProperties()
      ...
      .forEach((prop) => {
        $(prop.selector).css({ "background-color": "#dfd" });
        $(`a[href="#venue-edit-${prop.tab}"]`).css({
...
const _SHORTCUT = {
  ...
  add(shortcutCombo, callback, opt) {
    ...
    this.all_shortcuts[shortcutCombo] = {
      callback: func,
      target: ele,
      event: opt.type,
    };
...
function isAlwaysOpen(venue) {
  ...
  return (
    hours.length === 1 && hours[0].days.length === 7 && hours[0].isAllDay()
  );
}
...
function addPURWebSearchButton() {
  ...
  function openWebSearch() {
    ...
    if ($("#WMEPH-WebSearchNewTab").prop("checked")) {
      ...
      window.open(
        buildSearchUrl(newName, addr),
        _SEARCH_RESULTS_WINDOW_NAME,
...
function makeNameCheckList(pnhData) {
  ...
  const tighten = (str) =>
    str
      ...
      .replace(/^THE /g, "")
      .replace(/[^A-Z0-9]/g, "");
  const stripNonAlphaKeepCommas = (str) =>
  ...
  return pnhData.map((entry) => {
    ...
    if (splits[disableIdx] !== "1" || specCase.includes("betaEnable")) {
      ...
      if (!isNullOrWhitespace(nameBaseStr)) {
        // If base terms exist, otherwise only the primary name is matched
        newNameList = newNameList.concat(
          stripNonAlphaKeepCommas(nameBaseStr).split(",")
        );
      ...
      const category = splits[category1Idx]
        .toUpperCase()
        .replace(/[^A-Z0-9]/g, "");
      const appendWords = [];
      if (category === "HOTEL") {
        ...
        category === "BANKFINANCIAL" &&
        !/\bnotABank\b/.test(specCase)
      ) {
      ...
      _wordVariations.forEach((variationsList) =>
        addSpellingVariants(newNameList, variationsList)
      );

      return _.uniq(newNameList)
        .join("|")
        .replace(/\|{2,}/g, "|")
        .replace(/\|+$/g, "");
    } // END if valid line
...
function saveWhitelistToLS(compress) {
  ...
  if (compress) {
    if (_venueWhitelistStr.length < 4800000) {
...
function backupWhitelistToLS(compress) {
  ...
  if (compress) {
    ...
    localStorage.setItem(
      _WL_LOCAL_STORE_NAME_COMPRESSED + Math.floor(Date.now() / 1000),
      _venueWhitelistStr
    );
  } else {
    localStorage.setItem(
      _WL_LOCAL_STORE_NAME + Math.floor(Date.now() / 1000),
      _venueWhitelistStr
    );
  }
}
...
function whitelistAction(itemID, wlKeyName) {
  ...
  const itemGPS = OpenLayers.Layer.SphericalMercator.inverseMercator(
    venue.attributes.geometry.getCentroid().x,
    venue.attributes.geometry.getCentroid().y
  );
  if (!_venueWhitelist.hasOwnProperty(itemID)) {
...
function harmoList(
  ...
  for (let pnhIdx = 1, len = pnhNames.length; pnhIdx < len; pnhIdx++) {
    ...
    if (specCases.includes("regexNameMatch")) {
      ...
      if (pnhEntrySplits[phSearchNameWordIdx] !== "") {
        spaceMatchList.push(
          ...pnhEntrySplits[phSearchNameWordIdx]
            .toUpperCase()
            .replace(/, /g, ",")
            .split(",")
        );
      ...
      if (specCases.includes("strMatchStart")) {
        ...
        for (let nmix = 0; nmix < nameComps.length; nmix++) {
          if (
            itemName.startsWith(nameComps[nmix]) ||
            itemNameNoNum.startsWith(nameComps[nmix])
          ) {
            PNHStringMatch = true;
          }
        }
        ...
        for (let nmix = 0; nmix < nameComps.length; nmix++) {
          if (
            itemName.endsWith(nameComps[nmix]) ||
            itemNameNoNum.endsWith(nameComps[nmix])
          ) {
            PNHStringMatch = true;
          }
        }
      } else if (
        nameComps.includes(itemName) ||
        nameComps.includes(itemNameNoNum)
      ) {
    ...
    if (PNHStringMatch) {
      ...
      if (PNHMatchProceed) {
        ...
        approvedRegions = pnhEntrySplits[phRegionIdx]
          .replace(/ /g, "")
          .toUpperCase()
          .split(",");

        if (
          approvedRegions.includes(state2L) ||
          approvedRegions.includes(region3L) || // if the WME-selected item matches the state, region
  ...
  if (pnhNameMatch) {
    ...
    _buttonBanner.ApprovalSubmit = new Flag.ApprovalSubmit(
      region3L,
      pnhOrderNum,
      pnhNameTemp,
      placePL
...
function onObjectsChanged() {
  ...
  if ($("#WMEPH_banner").length > 0 && venue) {
    ...
    if (
      lastAction &&
      lastAction.object &&
      lastAction.object.type === "venue" &&
      lastAction.attributes &&
      lastAction.attributes.id === venue.attributes.id
    ) {
      if (
        lastAction.newAttributes &&
...
function toggleXrayMode(enable) {
  ...
  if (enable) {
    _layer.styleMap.styles.default.rules = _layer.styleMap.styles.default.rules.filter(
      ...
      filter: new OpenLayers.Filter.Comparison({
        ...
        evaluate(venue) {
          return (
            venue &&
            venue.model &&
            venue.model.attributes.wmephSeverity === this.value
          );
        },
      }),
      symbolizer,
      ...
      filter: new OpenLayers.Filter.Comparison({
        ...
        evaluate(venue) {
          if (
            ...
            venue &&
            venue.model &&
            venue.model.attributes.categories &&
            venue.model.attributes.categoryAttributes &&
            venue.model.attributes.categoryAttributes.PARKING_LOT &&
            venue.model.attributes.categories.includes("PARKING_LOT")
          ) {
            const type =
              venue.model.attributes.categoryAttributes.PARKING_LOT.parkingType;
            return (
...
function applyHighlightsTest(venues, force) {
  ...
  venues.forEach((venue) => {
    if (venue && venue.type === "venue" && venue.attributes) {
      ...
      if (
        doHighlight &&
        !(disableRankHL && venue.attributes.lockRank > _USER.rank - 1)
      ) {
        try {
...
function bootstrapWmephColorHighlights() {
  if (localStorage.getItem("WMEPH-ColorHighlighting") === "1") {
    ...
    W.map.venueLayer.events.register("beforefeaturesadded", null, (e) =>
      errorHandler(() => applyHighlightsTest(e.features.map((f) => f.model)))
    );
...
function toTitleCaseStrong(str) {
  ...
  str = str
    ...
    .replace(/[^ ]+/g, (txt) => {
      ...
      return (
        _TITLECASE_SETTINGS.specWords.find(
...
function normalizePhone(s, outputFormat, returnType, item, region) {
  ...
  if (returnType === "inputted") {
    try {
      const phonenumber = new libphonenumber.parsePhoneNumberFromString(
        s,
        Country()
      );
      if (phonenumber) {
...
function executeMultiAction(actions, description) {
  if (actions.length > 0) {
    ...
    mAction._description =
      description || mAction._description || "Change(s) made by WMEPH";
...
function normalizeURL(s, lc, skipBannerActivate, venue, region) {
  ...
  if ((!s || s.trim().length === 0) && !skipBannerActivate) {
    ...
    const hasOperator =
      venue.attributes.brand &&
      W.model.categoryBrands.PARKING_LOT.includes(venue.attributes.brand);
    if (
      !venue.isParkingLot() ||
      (venue.isParkingLot() &&
        (regionsThatWantPLAUrls.includes(region) || hasOperator))
    ) {
  ...
  if (!s || s.trim().length === 0 || !/(^https?:\/\/)?\w+\.\w+/.test(s))
    s = "badURL";
  return s;
...
let Flag = {
  ...
  FullAddressInference: class extends FlagBase {
    constructor() {
      super(
        true,
        3,
        "Missing address was inferred from nearby segments. Verify the address and run script again."
      );
    }
    ...
    static eval(venue, addr, actions) {
      ...
      if (!addr.state || !addr.country) {
        if (W.map.getZoom() < 4) {
          ...
          if (inferredAddress && inferredAddress.attributes)
            inferredAddress = inferredAddress.attributes;
          // Use the state when needed
          if (
            inferredAddress &&
            ((usesStates() && inferredAddress.state) || !usesStates()) &&
            inferredAddress.country
          ) {
            if ($("#WMEPH-AddAddresses").prop("checked")) {
              // update the item's address if option is enabled
  ...
  PlaNameMissing: class extends FlagBase {
    constructor() {
      ...
      this.message +=
        _USER.rank < 3
          ? " Request an R3+ lock to confirm unnamed parking lot."
          : " Lock to 3+ to confirm unnamed parking lot.";
    }
  },
  ...
  IndianaLiquorStoreHours: class extends WLFlag {
    ...
    static eval(venue, name, hpMode) {
      ...
      if (
        hpMode.harmFlag &&
        !_wl.indianaLiquorStoreHours &&
        [/\bbeers?\b/, /\bwines?\b/, /\bliquor\b/, /\bspirits\b/].some((re) =>
          re.test(name)
        ) &&
        !venue.attributes.openingHours.some((entry) =>
          entry.days.includes(0)
        ) &&
        !venue.isResidential()
      ) {
  ...
  HoursOverlap: class extends FlagBase {
    constructor() {
      super(true, 3, "Overlapping hours of operation. Place might not save.");
    }
  },
  UnmappedRegion: class extends WLFlag {
    constructor() {
      super(
        true,
        3,
        "This category is usually not mapped in this region.",
        true,
        "Whitelist unmapped category",
        "unmappedRegion"
      );
    }
  },
  ...
  GasUnbranded: class extends FlagBase {
    ...
    constructor() {
      super(
        true,
        3,
        '"Unbranded" should not be used for the station brand. Change to correct brand or ' +
  ...
  GasMkPrim: class extends ActionFlag {
    constructor() {
      super(
        true,
        3,
        "Gas Station is not the primary category",
        "Fix",
  ...
  IsThisAPilotTravelCenter: class extends ActionFlag {
    constructor() {
      super(true, 0, 'Is this a "Travel Center"?', "Yes", "");
    }
  ...
  HotelMkPrim: class extends WLActionFlag {
    ...
    action() {
      ...
      const categories = insertAtIX(
        venue.attributes.categories.slice(),
        "HOTEL",
        0
      );
      _UPDATED_FIELDS.categories.updated = true;
  ...
  ChangeToPetVet: class extends WLActionFlag {
    ...
    static eval(name, categories) {
      ...
      if (
        (categories.includes("HOSPITAL_URGENT_CARE") ||
          categories.includes("DOCTOR_CLINIC")) &&
        (containsAny(testNameWords, _PNH_DATA[country]._animalFullMatch) ||
          _PNH_DATA[country]._animalPartMatch.some((match) =>
            testName.includes(match)
          ))
      ) {
  ...
  NotASchool: class extends WLFlag {
    ...
    static eval(name, categories) {
      ...
      if (
        ...
        (containsAny(testNameWords, _PNH_DATA[Country()]._schoolFullMatch) ||
          _PNH_DATA[Country()]._schoolPartMatch.some((match) =>
            testName.includes(match)
          ))
      ) {
  ...
  HnMissing: class extends WLActionFlag {
    ...
    action() {
      ...
      if (hnTemp > 0 && hnTemp < 1000000) {
        const action = new UpdateObject(this.venue, {
  ...
  HnTooManyDigits: class extends WLFlag {
    constructor() {
      super(
        true,
        2,
        "HN contains more than 6 digits. Please verify.",
        true,
        "Whitelist long HN",
        "hnTooManyDigits"
      );
  ...
  CityMissing: class extends ActionFlag {
    constructor() {
      super(true, 3, "No city:", "Edit address", "Edit address to add city.");
    }
  ...
  BankBranch: class extends ActionFlag {
    constructor() {
      super(
        true,
        1,
        "Is this a bank branch office? ",
        "Yes",
        "Is this a bank branch?"
      );
    }
    ...
    action() {
      ...
      const tempName = _newName
        .replace(/[- (]*ATM[- )]*/g, " ")
        .replace(/^ /g, "")
        .replace(/ $/g, ""); // strip ATM from name if present
      _newName = tempName;
      W.model.actionManager.add(
        new UpdateObject(venue, { name: _newName, categories: _newCategories })
      );
  ...
  BankCorporate: class extends ActionFlag {
    constructor() {
      super(
        true,
        1,
        "Or is this the bank's corporate offices?",
        "Yes",
        "Is this the bank's corporate offices?"
      );
    }
    ...
    action() {
      ...
      const tempName = _newName
        ...
        .replace(/^ /g, "")
        .replace(/ $/g, "")
        .replace(/ {2,}/g, " "); // strip ATM from name if present
      _newName = tempName;
      W.model.actionManager.add(
        new UpdateObject(venue, {
          name: `${_newName} - Corporate Offices`,
  ...
  IgnEdited: class extends FlagBase {
    constructor() {
      super(true, 2, "Last edited by an IGN editor");
    }
  },
  ...
  Overlapping: class extends FlagBase {
    constructor() {
      super(true, 2, "Place points are stacked up.");
    }
  },
  ...
  LongURL: class extends WLActionFlag {
    ...
    action() {
      ...
      if (!isNullOrWhitespace(_tempPNHURL)) {
        W.model.actionManager.add(
          new UpdateObject(venue, { url: _tempPNHURL })
        );
  ...
  FormatUSPS: class extends FlagBase {
    constructor() {
      super(
        true,
        1,
        'Name the post office according to this region\'s <a href="https://wazeopedia.waze.com/wiki/USA/Places/Post_Office" style="color:#3232e6" target="_blank"> standards for USPS post offices</a>'
      );
    }
  },
  ...
  PnhCatMess: class extends ActionFlag {
    constructor(message) {
      super(true, 0, message, null, null);
    }
  ...
  ExtProviderMissing: class extends ActionFlag {
    ...
    static eval(
      ...
      if (
        userRank >= 2 &&
        venue.areExternalProvidersEditable() &&
        !(venue.isParkingLot() && ignoreParkingLots)
      ) {
        const catsToIgnore = [
          "BRIDGE",
          "TUNNEL",
          ...
          if (!(provIDs && provIDs.length)) {
            ...
            if (isLocked) {
              ...
              const weeksSinceLastUpdate =
                (Date.now() - lastUpdated) / 604800000;
              if (
    ...
    action2() {
      ...
      setTimeout(() => {
        ...
        $("div.external-providers-view > div > ul > div > div > li > div > a")
          .last()
          .mousedown();
        setTimeout(
          () =>
  ...
  BadAreaCode: class extends WLActionFlag {
    constructor(textValue, outputFormat) {
      super(
        true,
        1,
        `Area Code mismatch:<br><input type="text" id="WMEPH-PhoneAdd" autocomplete="off" style="font-size:0.85em;width:100px;padding-left:2px;color:#000;" value="${
          textValue || ""
        }">`,
        "Update",
        "Update phone #",
  ...
  PhoneMissing: class extends WLActionFlag {
    ...
    static get _regionsThatWantPlaPhones() {
      return ["SER"];
    }
    ...
    static eval(venue, wl, region, outputFormat) {
      ...
      if (
        !isPLA ||
        (isPLA &&
          (this._regionsThatWantPlaPhones.includes(region) || hasOperator))
      ) {
        flag = new Flag.PhoneMissing(
          venue,
          hasOperator,
          wl,
          outputFormat,
    ...
    action() {
      ...
      if (newPhone === "badPhone") {
        ...
        phlogdev(newPhone);
        W.model.actionManager.add(
          new UpdateObject(this.venue, { phone: newPhone })
        );
        _UPDATED_FIELDS.phone.updated = true;
  ...
  NoHours: class extends WLFlag {
    ...
    applyHours(replaceAllHours) {
      ...
      if (
        ...
        W.model.actionManager.add(
          new UpdateObject(venue, { openingHours: parseResult.hours })
        );
        _UPDATED_FIELDS.openingHours.updated = true;
        ...
        $("#WMEPH-HoursPaste")
          ...
          .attr({ title: this.getTitle(parseResult) });
      }
    }
          ...
          if (hpMode.harmFlag) {
            ...
            result.flag.message += [
              ...
              .map((btnInfo) =>
                $("<button>", {
                  class:
                    "wmeph-pla-lot-type-btn btn btn-default btn-xs wmeph-btn",
        ...
        if (
          ...
          if (hpMode.harmFlag) {
            [
              ...
              result.flag.message += $("<button>", {
                id: `wmeph_${btnInfo[0]}`,
                class:
                  "wmeph-pla-cost-type-btn btn btn-default btn-xs wmeph-btn",
                title: btnInfo[2],
              })
                .text(btnInfo[1])
                .css({
  ...
  NoPlaStopPoint: class extends ActionFlag {
    constructor() {
      super(
        true,
        1,
        "Entry/exit point has not been created.",
        "Add point",
        "Add an entry/exit point"
      );
    }
    ...
    static eval(venue) {
      ...
      if (
        venue.isParkingLot() &&
        (!venue.attributes.entryExitPoints ||
          !venue.attributes.entryExitPoints.length)
      ) {
  ...
  PlaStopPointUnmoved: class extends FlagBase {
    constructor() {
      super(true, 1, "Entry/exit point has not been moved.");
    }
    ...
    static eval(venue) {
      ...
      if (
        venue.isParkingLot() &&
        attr.entryExitPoints &&
        attr.entryExitPoints.length
      ) {
  ...
  PlaCanExitWhileClosed: class extends ActionFlag {
    constructor() {
      super(true, 0, "Can cars exit when lot is closed? ", "Yes", "");
    }
    ...
    static eval(venue, hpMode) {
      ...
      if (hpMode.harmFlag && venue.isParkingLot()) {
        ...
        if (
          parkAttr &&
          !parkAttr.canExitWhileClosed &&
          ($("#WMEPH-ShowPLAExitWhileClosed").prop("checked") ||
            !(
              isAlwaysOpen(venue) || venue.attributes.openingHours.length === 0
            ))
        ) {
  ...
  PlaHasAccessibleParking: class extends ActionFlag {
    constructor() {
      super(true, 0, "Does this lot have disability parking? ", "Yes", "");
    }
  ...
  AllDayHoursFixed: class extends FlagBase {
    constructor() {
      super(true, 0, 'Hours were changed from 00:00-23:59 to "All Day"');
    }
    ...
    static eval(venue, hpMode, actions) {
      ...
      for (let i = 0, len = hoursEntries.length; i < len; i++) {
        const newHoursEntry = new OpeningHour({
          days: [].concat(hoursEntries[i].days),
          fromHour: hoursEntries[i].fromHour,
          toHour: hoursEntries[i].toHour,
        });
        if (
          newHoursEntry.toHour === "23:59" &&
          /^0?0:00$/.test(newHoursEntry.fromHour)
        ) {
          if (hpMode.hlFlag) {
  ...
  LocalURL: class extends FlagBase {
    constructor() {
      super(
        true,
        0,
        "Some locations for this business have localized URLs, while others use the primary corporate site. Check if a local URL applies to this location."
      );
    }
  },
  LockRPP: class extends ActionFlag {
    constructor() {
      super(
        true,
        0,
        "Lock this residential point?",
        "Lock",
        "Lock the residential point"
      );
    }
    ...
    action() {
      const venue = getSelectedVenue();
      let RPPlevelToLock =
        $("#RPPLockLevel :selected").val() || _defaultLockLevel + 1;
      phlogdev(`RPPlevelToLock: ${RPPlevelToLock}`);

      RPPlevelToLock -= 1;
      W.model.actionManager.add(
        new UpdateObject(venue, { lockRank: RPPlevelToLock })
      );
      // no field highlight here
      this.message = `Current lock: ${
        parseInt(venue.attributes.lockRank, 10) + 1
      }. ${_rppLockString} ?`;
    }
  },
  AddAlias: class extends ActionFlag {
    constructor(specCases, optionalAlias) {
      super(
        true,
        0,
        `Is there a ${optionalAlias} at this location?`,
        "Yes",
        `Add ${optionalAlias}`
      );
    ...
    action() {
      const venue = getSelectedVenue();
      let aliases = insertAtIX(
        venue.attributes.aliases.slice(),
        this.optionalAlias,
        0
      );
      if (
        this.specCases.includes("altName2Desc") &&
        !venue.attributes.description
          .toUpperCase()
          .includes(this.optionalAlias.toUpperCase())
      ) {
  ...
  AddCat2: class extends ActionFlag {
    ...
    action() {
      ...
      W.model.actionManager.add(
        new UpdateObject(venue, { categories: _newCategories })
      );
      _UPDATED_FIELDS.categories.updated = true;
      harmonizePlaceGo(venue, "harmonize");
    }
  ...
  AddPharm: class extends ActionFlag {
    constructor() {
      super(
        true,
        0,
        "Is there a Pharmacy at this location?",
        "Yes",
  ...
  AddSuper: class extends ActionFlag {
    constructor() {
      super(
        true,
        0,
        "Does this location have a supermarket?",
        "Yes",
        "Add Supermarket category"
      );
    }
  ...
  AppendAMPM: class extends ActionFlag {
    constructor() {
      super(
        true,
        0,
        "Is there an ampm at this location?",
        "Yes",
        "Add ampm to the place"
      );
    }
  ...
  AddATM: class extends ActionFlag {
    constructor() {
      super(
        ...
        "ATM at location? ",
        "Yes",
        "Add the ATM category to this place"
      );
    }
  ...
  AddConvStore: class extends ActionFlag {
    constructor() {
      super(
        true,
        0,
        "Add convenience store category? ",
        "Yes",
  ...
  ChangeToHospitalUrgentCare: class extends WLActionFlag {
    ...
    static eval(venue, hpMode) {
      ...
      if (
        hpMode.harmFlag &&
        venue.attributes.categories.includes("DOCTOR_CLINIC")
      ) {
        result.flag = new Flag.ChangeToHospitalUrgentCare(
          0,
  ...
  NotAHospital: class extends WLActionFlag {
    ...
    static eval(categories) {
      ...
      if (categories.includes("HOSPITAL_URGENT_CARE")) {
        ...
        const testNameWords = testName.split(" ");
        if (
          containsAny(testNameWords, _PNH_DATA[Country()]._hospitalFullMatch) ||
          _PNH_DATA[Country()]._hospitalPartMatch.some((match) =>
            testName.includes(match)
          )
        ) {
          if (!_wl.notAHospital) {
  ...
  ChangeToDoctorClinic: class extends WLActionFlag {
    ...
    static eval(venue, categories, hpMode, pnhNameRegMatch) {
      ...
      if (
        ...
        ((categories.includes("PERSONAL_CARE") && !pnhNameRegMatch) ||
          _newCategories.includes("OFFICES"))
      ) {
    ...
    action() {
      ...
      if (newName === this.originalName || this.confirmChange) {
        ...
        if (parts.base !== newName) {
          W.model.actionManager.add(
            new UpdateObject(venue, { name: newName + (parts.suffix || "") })
          );
          _UPDATED_FIELDS.name.updated = true;
        }
        harmonizePlaceGo(venue, "harmonize");
  ...
  SFAliases: class extends FlagBase {
    constructor() {
      super(true, 0, "Unnecessary aliases were removed.");
    }
  },
  PlaceMatched: class extends FlagBase {
    constructor() {
      super(true, 0, "Place matched from PNH data.");
    }
  },
  PlaceLocked: class extends FlagBase {
    constructor() {
      super(true, 0, "Place locked.");
    }
  },
  NewPlaceSubmit: class extends ActionFlag {
    constructor() {
      super(
        true,
        0,
        "No PNH match. If it's a chain: ",
        "Submit new chain data",
  ...
  ApprovalSubmit: class extends ActionFlag {
    ...
    action() {
      if (
        _PM_USER_LIST.hasOwnProperty(this.region) &&
        _PM_USER_LIST[this.region].approvalActive
      ) {
        const forumPMInputs = {
        ...
        newForumPost(
          "https://www.waze.com/forum/ucp.php?i=pm&mode=compose",
          forumPMInputs
        );
      } else {
  ...
  PlaceWebsite: class extends ActionFlag {
    ...
    action() {
      ...
      if (_updateURL) {
        ...
        if (
          localStorage.getItem(_SETTING_IDS.sfUrlWarning) === "0" &&
          _customStoreFinderLocal
        ) {
          WazeWrap.Alerts.confirm(
            _SCRIPT_NAME,
...
function harmonizePlaceGo(item, useFlag, actions) {
  ...
  _buttonBanner.allDayHoursFixed = Flag.AllDayHoursFixed.eval(
    item,
    hpMode,
    actions
  );
  ...
  if (hpMode.harmFlag && item.isParkingLot())
  ...
  _buttonBanner.plaCanExitWhileClosed = Flag.PlaCanExitWhileClosed.eval(
    item,
    hpMode
  ).flag;
  _buttonBanner.plaPaymentTypeMissing = Flag.PlaPaymentTypeMissing.eval(
    item
  ).flag;
  _buttonBanner.plaHasAccessibleParking = Flag.PlaHasAccessibleParking.eval(
    item,
    hpMode
  ).flag;

  // Check categories that maybe should be Hospital / Urgent Care, or Doctor / Clinic.
  _buttonBanner.changeToHospitalUrgentCare = Flag.ChangeToHospitalUrgentCare.eval(
    item,
    hpMode
  ).flag;
  ...
  let itemGPS;
  if (
    _venueWhitelist.hasOwnProperty(itemID) &&
    (hpMode.harmFlag ||
      (hpMode.hlFlag && !$("#WMEPH-DisableWLHL").prop("checked")))
  ) {
    // Enable the clear WL button if any property is true
    Object.keys(_venueWhitelist[itemID]).forEach((wlKey) => {
      // loop thru the venue WL keys
      if (
        _venueWhitelist[itemID].hasOwnProperty(wlKey) &&
        (_venueWhitelist[itemID][wlKey].active || false)
      ) {
    ...
    if (
      _venueWhitelist[itemID].hasOwnProperty("dupeWL") &&
      _venueWhitelist[itemID].dupeWL.length > 0
    ) {
      if (hpMode.harmFlag) _buttonBanner2.clearWL.active = true;
    ...
    if (hpMode.harmFlag) {
      ...
      if (!itemGPS) {
        ...
        itemGPS = OpenLayers.Layer.SphericalMercator.inverseMercator(
          centroid.x,
          centroid.y
        );
      }
  ...
  if (
    hpMode.harmFlag &&
    (addr.county === null || (usesStates && addr.state === null))
  ) {
    WazeWrap.Alerts.error(
      _SCRIPT_NAME,
  ...
  if (addr.country.abbr in _PNH_DATA) {
    ...
    if (hpMode.harmFlag) {
      WazeWrap.Alerts.error(
        _SCRIPT_NAME,
        "At present this script is not supported in this country."
      );
    }
    return 3;
  }
  ...
  result = Flag.IsThisAPilotTravelCenter.eval(
    item,
    hpMode,
    state2L,
    _newName,
  ...
  if (item.isGasStation()) {
    ...
    if (
      hpMode.harmFlag &&
      (!_newName || _newName.trim().length === 0) &&
    ...
    if (
      !_newCategories.includes("CONVENIENCE_STORE") &&
      !_buttonBanner.subFuel
    ) {
  ...
  _buttonBanner.indianaLiquorStoreHours = Flag.IndianaLiquorStoreHours.eval(
    item,
    _newName,
    hpMode
  ).flag;
  ...
  const isLocked =
    item.attributes.lockRank >=
    (_pnhLockLevel > -1 ? _pnhLockLevel : _defaultLockLevel);
  ...
  if (item.attributes.residential) {
  ...
  } else if (item.isParkingLot() || (_newName && _newName.trim().length)) {
    ...
    if (
      ...
      const orderList = [];
      if (pnhMatchData.length > 1) {
        // If multiple matches, then
        let brandParent = -1;
        let pnhMatchDataHold = pnhMatchData[0].split("|");
        for (let pmdix = 0; pmdix < pnhMatchData.length; pmdix++) {
          ...
          if (pmdTemp[phSpecCaseIdx].match(/brandParent(\d{1})/) !== null) {
            // If there is a brandParent flag, prioritize by highest match
            const [, pmdSpecCases] = pmdTemp[phSpecCaseIdx].match(
              /brandParent(\d{1})/
            );
            if (pmdSpecCases > brandParent) {
            ...
            // if any item has no brandParent structure, use highest brandParent match but post an error
            nsMultiMatch = true;
          }
        }
      ...
      if (phSpecCaseIdx > -1) {
        ...
        for (let scix = 0; scix < specCases.length; scix++) {
          ...
          if ((match = specCase.match(/^buttOn_(.+)/i))) {
            ...
            switch (scFlag) {
              ...
              default:
                console.error(
                  "WMEPH:",
                  `Could not process specCase value: buttOn_${scFlag}`
                );
            }
          ...
          if (
            (match = /forceBrand<>([^,<]+)/i.exec(pnhMatchData[phSpecCaseIdx]))
          ) {
            [, newBrand] = match;
          }
          ...
          if (
            ["GAS_STATION"].includes(priPNHPlaceCat) &&
            (match = specCase.match(/^forceBrand<>(.+)/i))
          ) {
          ...
          if (
            phDisplayNoteIdx > -1 &&
            !isNullOrWhitespace(pnhMatchData[phDisplayNoteIdx])
          ) {
            displayNote = pnhMatchData[phDisplayNoteIdx];
          }
          result = Flag.LocalizedName.eval(
            _newName,
            newNameSuffix,
            specCase,
            displayNote
      ...
      if (
        ...
        !_newName.toUpperCase().includes("GAS") &&
        !_newName.toUpperCase().includes("FUEL")
      ) {
      ...
      if (
        ...
        if (containsAny(specCases, ["pharmhours"])) {
          if (
            ...
            (!item.attributes.description.toUpperCase().includes("HOURS") &&
              !item.attributes.description.toUpperCase().includes("HRS"))
          ) {
            _buttonBanner.specCaseMessage = new Flag.SpecCaseMessage(
              pnhMatchData[phDisplayNoteIdx]
            );
          }
          ...
          if (
            ...
            if ($("#service-checkbox-DRIVETHROUGH").prop("checked")) {
              ...
              _buttonBanner.specCaseMessageLow = new Flag.SpecCaseMessageLow(
                pnhMatchData[phDisplayNoteIdx]
              );
            }
          }
      ...
      if (phStoreFinderUrlIdx > -1) {
        // if the sfurl column exists...
        if (
          phStoreFinderUrlLocalIdx > -1 &&
          !isNullOrWhitespace(pnhMatchData[phStoreFinderUrlLocalIdx])
        ) {
          ...
          const tempLocalURL = pnhMatchData[phStoreFinderUrlLocalIdx]
            .replace(/ /g, "")
            .split("<>");
          let searchStreet = "";
          let searchCity = "";
          ...
          for (let tlix = 1; tlix < tempLocalURL.length; tlix++) {
            if (tempLocalURL[tlix] === "ph_streetName") {
              ...
              _customStoreFinderLocalURL += item.attributes.houseNumber
                ? item.attributes.houseNumber
                : "";
            } else {
      ...
      if (altCategories && altCategories.length) {
        ...
        altCategories = altCategories
          .replace(/,[^A-Za-z0-9]*/g, ",")
          .split(","); // tighten and split by comma
        for (let catix = 0; catix < altCategories.length; catix++) {
          ...
          if (newAltTemp === "ERROR") {
            // if no translation, quit the loop
      ...
      if (["HOTEL"].includes(priPNHPlaceCat)) {
        ...
        if (
          nameToCheck.toUpperCase() === pnhMatchData[phNameIdx].toUpperCase()
        ) {
          // If no localization
          ...
          const splix = _newName
            .toUpperCase()
            .replace(/[-/]/g, " ")
            .indexOf(
              pnhMatchData[phNameIdx].toUpperCase().replace(/[-/]/g, " ")
            );
          if (splix > -1) {
            const frontText = _newName.slice(0, splix);
            const backText = _newName.slice(
              splix + pnhMatchData[phNameIdx].length
            );
            _newName = pnhMatchData[phNameIdx];
            if (frontText.length > 0) {
              _newName = `${frontText} ${_newName}`;
            }
            if (backText.length > 0) {
              _newName = `${_newName} ${backText}`;
            }
        ...
        if (_newCategories.indexOf("HOTEL") !== 0) {
          // If no HOTEL category in the primary, flag it
        ...
        _newCategories.includes("BANK_FINANCIAL") &&
        !pnhMatchData[phSpecCaseIdx].includes("notABank")
      ) {
        // PNH Bank treatment
        ...
        if (/\batm\b/gi.test(_newName)) {
          ...
          // if no ATM in name but with a banking category:
          if (_ixOffices === 0) {
            _buttonBanner.bankBranch = new Flag.BankBranch();
          } else if (_ixBank > -1 && _ixATM === -1) {
          ...
          // for PNH match with neither bank type category, make it a bank
          _newCategories = insertAtIX(_newCategories, "BANK_FINANCIAL", 1);
          _buttonBanner.standaloneATM = new Flag.StandaloneATM();
          _buttonBanner.bankCorporate = new Flag.BankCorporate();
        } // END PNH bank treatment
      } else if (["GAS_STATION"].includes(priPNHPlaceCat)) {
        // for PNH gas stations, don't replace existing sub-categories
        if (altCategories && altCategories.length) {
          // if PNH alts exist
          insertAtIX(_newCategories, altCategories, 1); //  then insert the alts into the existing category array after the GS category
        }
        if (_newCategories.indexOf("GAS_STATION") !== 0) {
          // If no GS category in the primary, flag it
        ...
        if (
          ...
          !specCases.includes("buttOn_addCat2") &&
          !specCases.includes("optionCat2")
        ) {
        ...
        if (_newName !== titleCaseName) {
          ...
          _buttonBanner.STC.suffixMessage = `<span style="margin-left: 4px;font-size: 14px">&bull; ${titleCaseName}${
            newNameSuffix || ""
          }</span>`;
      ...
      if (localURLcheck !== "") {
        if (_newURL !== null || _newURL !== "") {
          ...
          if (_newURL.match(localURLcheckRE) !== null) {
            ...
            _newURL = normalizeURL(
              pnhMatchData[phUrlIdx],
              false,
              true,
              item,
              region
      ...
      if (!isNullOrWhitespace(_newAliasesTemp)) {
        // make aliases array
      ...
      if (
        !specCases.includes("noUpdateAlias") &&
        !containsAll(_newAliases, _newAliasesTemp) &&
        _newAliasesTemp &&
        _newAliasesTemp.length &&
        !specCases.includes("optionName2")
      ) {
        _newAliases = insertAtIX(_newAliases, _newAliasesTemp, 0);
      }

      // Remove unnecessary parent categories
      const catData = _PNH_DATA[Country()].categories.map((cat) =>
        cat.split("|")
      );
      const catParentIdx = catData[0].indexOf("pc_catparent");
      const catNameIdx = catData[0].indexOf("pc_wmecat");
      const parentCats = _.uniq(
        _newCategories.map(
          (catName) =>
            catData.find((cat) => cat[catNameIdx] === catName)[catParentIdx]
        )
      ).filter((parent) => parent.trim(" ").length > 0);
      _newCategories = _newCategories.filter(
        (cat) => !parentCats.includes(cat)
      );

      // update categories if different and no Cat2 option
      if (
        !matchSets(_.uniq(item.attributes.categories), _.uniq(_newCategories))
      ) {
        if (
          !specCases.includes("optionCat2") &&
          !specCases.includes("buttOn_addCat2")
      ...
      if (
        specCases.includes("buttOn_addCat2") &&
        !_newCategories.includes(altCategories[0])
      ) {
      ...
      if (
        !isNullOrWhitespace(newDescripion) &&
        !item.attributes.description
          .toUpperCase()
          .includes(newDescripion.toUpperCase())
      ) {
        if (
          item.attributes.description !== "" &&
          item.attributes.description !== null &&
      ...
      _ixOffices = item.attributes.categories.indexOf("OFFICES");
      // if the name contains ATM in it
      if (_newName.match(/\batm\b/gi) !== null) {
        if (_ixOffices === 0) {
          _buttonBanner.bankType1 = new Flag.BankType1();
          _buttonBanner.bankBranch = new Flag.BankBranch();
          _buttonBanner.standaloneATM = new Flag.StandaloneATM();
    ...
    if (hpMode.harmFlag) {
      ...
      if (_newName + (newNameSuffix || "") !== item.attributes.name) {
        ...
        actions.push(
          new UpdateObject(item, { name: _newName + (newNameSuffix || "") })
        );
      ...
      if (
        _newAliases.some((alias) => !item.attributes.aliases.includes(alias)) ||
        _newAliases.length !== item.attributes.aliases.length
      ) {
      ...
      switch (region) {
        ...
        case "SWR":
          regionFormURL =
            "https://docs.google.com/forms/d/1Qf2N4fSkNzhVuXJwPBJMQBmW0suNuy8W9itCo1qgJL4/viewform";
        ...
        case "HI":
          regionFormURL =
            "https://docs.google.com/forms/d/1K7Dohm8eamIKry3KwMTVnpMdJLaMIyDGMt7Bw6iqH_A/viewform";
          newPlaceAddon = `?entry.1497446659=${encodedTempSubmitName}&entry.1970139752=${encodedUrlSubmit}&entry.1749047694=${suffix}`;
          approvalAddon = `?entry.1497446659=${pnhNameTempWeb}&entry.50214576=${approvalMessage}&entry.1749047694=${suffix}`;
          break;
        case "PLN":
          regionFormURL =
            "https://docs.google.com/forms/d/1ycXtAppoR5eEydFBwnghhu1hkHq26uabjUu8yAlIQuI/viewform";
          newPlaceAddon = `?entry.925969794=${encodedTempSubmitName}&entry.1970139752=${encodedUrlSubmit}&entry.1749047694=${suffix}`;
          approvalAddon = `?entry.925969794=${pnhNameTempWeb}&entry.50214576=${approvalMessage}&entry.1749047694=${suffix}`;
          break;
        case "SCR":
          regionFormURL =
            "https://docs.google.com/forms/d/1KZzLdlX0HLxED5Bv0wFB-rWccxUp2Mclih5QJIQFKSQ/viewform";
          newPlaceAddon = `?entry.925969794=${encodedTempSubmitName}&entry.1970139752=${encodedUrlSubmit}&entry.1749047694=${suffix}`;
          approvalAddon = `?entry.925969794=${pnhNameTempWeb}&entry.50214576=${approvalMessage}&entry.1749047694=${suffix}`;
          break;
        case "GLR":
          regionFormURL =
            "https://docs.google.com/forms/d/19btj-Qt2-_TCRlcS49fl6AeUT95Wnmu7Um53qzjj9BA/viewform";
          newPlaceAddon = `?entry.925969794=${encodedTempSubmitName}&entry.1970139752=${encodedUrlSubmit}&entry.1749047694=${suffix}`;
          approvalAddon = `?entry.925969794=${pnhNameTempWeb}&entry.50214576=${approvalMessage}&entry.1749047694=${suffix}`;
          break;
        case "SAT":
          regionFormURL =
            "https://docs.google.com/forms/d/1bxgK_20Jix2ahbmUvY1qcY0-RmzUBT6KbE5kjDEObF8/viewform";
        ...
        case "SER":
          regionFormURL =
            "https://docs.google.com/forms/d/1jYBcxT3jycrkttK5BxhvPXR240KUHnoFMtkZAXzPg34/viewform";
        ...
        case "ATR":
          regionFormURL =
            "https://docs.google.com/forms/d/1v7JhffTfr62aPSOp8qZHA_5ARkBPldWWJwDeDzEioR0/viewform";
          newPlaceAddon = `?entry.925969794=${encodedTempSubmitName}&entry.1970139752=${encodedUrlSubmit}&entry.1749047694=${suffix}`;
          approvalAddon = `?entry.925969794=${pnhNameTempWeb}&entry.50214576=${approvalMessage}&entry.1749047694=${suffix}`;
          break;
        case "NER":
          regionFormURL =
            "https://docs.google.com/forms/d/1UgFAMdSQuJAySHR0D86frvphp81l7qhEdJXZpyBZU6c/viewform";
          newPlaceAddon = `?entry.925969794=${encodedTempSubmitName}&entry.1970139752=${encodedUrlSubmit}&entry.1749047694=${suffix}`;
          approvalAddon = `?entry.925969794=${pnhNameTempWeb}&entry.50214576=${approvalMessage}&entry.1749047694=${suffix}`;
          break;
        case "NOR":
          regionFormURL =
            "https://docs.google.com/forms/d/1iYq2rd9HRd-RBsKqmbHDIEBGuyWBSyrIHC6QLESfm4c/viewform";
          newPlaceAddon = `?entry.925969794=${encodedTempSubmitName}&entry.1970139752=${encodedUrlSubmit}&entry.1749047694=${suffix}`;
          approvalAddon = `?entry.925969794=${pnhNameTempWeb}&entry.50214576=${approvalMessage}&entry.1749047694=${suffix}`;
          break;
        case "MAR":
          regionFormURL =
            "https://docs.google.com/forms/d/1PhL1iaugbRMc3W-yGdqESoooeOz-TJIbjdLBRScJYOk/viewform";
          newPlaceAddon = `?entry.925969794=${encodedTempSubmitName}&entry.1970139752=${encodedUrlSubmit}&entry.1749047694=${suffix}`;
          approvalAddon = `?entry.925969794=${pnhNameTempWeb}&entry.50214576=${approvalMessage}&entry.1749047694=${suffix}`;
          break;
        case "CA_EN":
          regionFormURL =
            "https://docs.google.com/forms/d/13JwXsrWPNmCdfGR5OVr5jnGZw-uNGohwgjim-JYbSws/viewform";
          newPlaceAddon = `?entry_839085807=${encodedTempSubmitName}&entry_1067461077=${encodedUrlSubmit}&entry_318793106=${_USER.name}&entry_1149649663=${encodedPlacePL}`;
          approvalAddon = `?entry_839085807=${pnhNameTempWeb}&entry_1125435193=${approvalMessage}&entry_318793106=${_USER.name}&entry_1149649663=${encodedPlacePL}`;
          break;
        case "QC":
          regionFormURL =
            "https://docs.google.com/forms/d/13JwXsrWPNmCdfGR5OVr5jnGZw-uNGohwgjim-JYbSws/viewform";
      ...
      if (_PNH_DATA[Country()].newPlaceUrl) {
        let addon = _PNH_DATA[Country()].newPlaceAddon;
        addon = addon.replace(
          "${encodedTempSubmitName}",
          encodedTempSubmitName
        );
        addon = addon.replace("${encodedUrlSubmit}", encodedUrlSubmit);
        addon = addon.replace("${_USER.name}", _USER.name);
        addon = addon.replace("${encodedPlacePL}", encodedPlacePL);
        _newPlaceURL = _PNH_DATA[Country()].newPlaceUrl + addon;
        _approveRegionURL = _PNH_DATA[Country()].newPlaceUrl + approvalAddon;
      } else {
    ...
    for (let ixPlaceCat = 0; ixPlaceCat < _newCategories.length; ixPlaceCat++) {
      ...
      if (ixPNHCat > -1) {
        ...
        const regPoint = catDataTemp[catDataHeaders.indexOf("pc_regpoint")]
          .replace(/,[^A-za-z0-9]*/g, ",")
          .split(",");
        const regArea = catDataTemp[catDataHeaders.indexOf("pc_regarea")]
          .replace(/,[^A-za-z0-9]*/g, ",")
          .split(",");
        if (
          regPoint.includes(state2L) ||
          regPoint.includes(region) ||
          regPoint.includes(_countryCode)
        ) {
          pvaPoint = "1";
          pvaArea = "";
        } else if (
          regArea.includes(state2L) ||
          regArea.includes(region) ||
          regArea.includes(_countryCode)
        ) {
          pvaPoint = "";
          pvaArea = "1";
        }
        ...
        if (
          _newCategories.includes("POST_OFFICE") &&
          /\b(?:cpu|vpo)\b/i.test(item.attributes.name)
        ) {
          pvaPoint = "1";
          pvaArea = "";
        }
        ...
        _buttonBanner.pnhCatMess = Flag.PnhCatMess.eval(
          catMessage,
          _newCategories,
          hpMode
        ).flag;
        // Unmapped categories
        const catRare = catDataTemp[catDataHeaders.indexOf("pc_rare")]
          .replace(/,[^A-Za-z0-9}]+/g, ",")
          .split(",");
        if (
          catRare.includes(state2L) ||
          catRare.includes(region) ||
          catRare.includes(_countryCode)
        ) {
          if (
            catDataTemp[0] === "OTHER" &&
            [
            ...
            if (!isLocked) {
              ...
              _buttonBanner.unmappedRegion.message =
                'The "Other" category should only be used if no other category applies.  Manually lock the place to override this flag.';
              lockOK = false;
            }
          } else {
            _buttonBanner.unmappedRegion = new Flag.UnmappedRegion();
        ...
        // Parent Category
        const catParent = catDataTemp[catDataHeaders.indexOf("pc_parent")]
          .replace(/,[^A-Za-z0-9}]+/g, ",")
          .split(",");
        if (
          catParent.includes(state2L) ||
          catParent.includes(region) ||
          catParent.includes(_countryCode)
        ) {
        ...
        for (let lockix = 1; lockix < 6; lockix++) {
          const catLockTemp = catDataTemp[
            catDataHeaders.indexOf(`pc_lock${lockix}`)
          ]
            .replace(/,[^A-Za-z0-9}]+/g, ",")
            .split(",");
          if (
            lockix - 1 > highestCategoryLock &&
            (catLockTemp.includes(state2L) ||
              catLockTemp.includes(region) ||
    ...
    if (item.attributes.openingHours.length === 0) {
      // if no hours...
      if (
        !containsAny(_newCategories, [
          "STADIUM_ARENA",
          "CEMETERY",
          "TRANSPORTATION",
          "FERRY_PIER",
      ...
      if (item.attributes.openingHours.length === 1) {
        ...
        if (
          hoursEntry.days.length < 7 &&
          /^0?0:00$/.test(hoursEntry.fromHour) &&
    ...
    if (_newURL !== item.attributes.url && !isNullOrWhitespace(_newURL)) {
      if (
        ...
        if (newURLTemp !== itemURLTemp) {
          // if formatted URLs don't match, then alert the editor to check the existing URL
    ...
    if (
      containsAny(["CA", "CO"], [region, state2L]) &&
      /^\d{3}-\d{3}-\d{4}$/.test(item.attributes.phone)
    ) {
      outputFormat = "{0}-{1}-{2}";
    } else if (
      region === "SER" &&
    ...
    if (!_wl.aCodeWL && (_countryCode === "USA" || _countryCode === "CAN")) {
      if (_newPhone !== null && _newPhone.match(/[2-9]\d{2}/) !== null) {
        ...
        if (!_areaCodeList.includes(areaCode)) {
          _buttonBanner.badAreaCode = new Flag.BadAreaCode(
            _newPhone,
            outputFormat
          );
        }
    ...
    if (_countryCode === "USA" && !_newCategories.includes("PARKING_LOT")) {
      if (!_newCategories.includes("POST_OFFICE")) {
        _buttonBanner.isThisAPostOffice = Flag.IsThisAPostOffice.eval(
          item,
          _newName
        ).flag;
      } else {
  ...
  if (_newCategories[0] === "GAS_STATION" && item.attributes.brand) {
    const compressedName = item.attributes.name
      .toUpperCase()
      .replace(/[^a-zA-Z0-9]/g, "");
    const compressedNewName = _newName
      .toUpperCase()
      .replace(/[^a-zA-Z0-9]/g, "");
    ...
    const compressedBrands = [
      newBrand.toUpperCase().replace(/[^a-zA-Z0-9]/g, ""),
    ];
    ...
    if (
      compressedBrands.every(
        (compressedBrand) =>
          !compressedName.includes(compressedBrand) &&
          !compressedNewName.includes(compressedBrand)
      )
    ) {
  ...
  // Name check
  if (
    !item.attributes.residential &&
    (!_newName || _newName.replace(/[^A-Za-z0-9]/g, "").length === 0)
  ) {
    if (item.isParkingLot()) {
      ...
      ![
        ...
        "RIVER_STREAM",
        "CANAL",
      ].includes(item.attributes.categories[0])
    ) {
  ...
  _buttonBanner.plaNameNonStandard = Flag.PlaNameNonStandard.eval(
    item,
    _wl
  ).flag;
  ...
  if (
    item.isParkingLot() &&
    item.attributes.categoryAttributes &&
    item.attributes.categoryAttributes.PARKING_LOT &&
  ...
  const hasStreet =
    item.attributes.streetID || (inferredAddress && inferredAddress.street);

  if (hasStreet && (!currentHN || currentHN.replace(/\D/g, "").length === 0)) {
    if (
      !"BRIDGE|ISLAND|FOREST_GROVE|SEA_LAKE_POOL|RIVER_STREAM|CANAL|DAM|TUNNEL|JUNCTION_INTERCHANGE"
        .split("|")
        .includes(item.attributes.categories[0])
    ) {
      ...
      if (
        state2L === "PR" ||
        ["SCENIC_LOOKOUT_VIEWPOINT"].includes(item.attributes.categories[0])
      ) {
    ...
    _buttonBanner.hnTooManyDigits = Flag.HnTooManyDigits.eval(
      currentHN,
      _wl
    ).flag;
  ...
  if (
    (!addr.city || addr.city.attributes.isEmpty) &&
    !"BRIDGE|ISLAND|FOREST_GROVE|SEA_LAKE_POOL|RIVER_STREAM|CANAL|DAM|TUNNEL|JUNCTION_INTERCHANGE"
      .split("|")
      .includes(item.attributes.categories[0])
  ) {
  ...
  result = Flag.ChangeToDoctorClinic.eval(
    item,
    _newCategories,
    hpMode,
    pnhNameRegMatch
  ...
  if (
    ["BRIDGE", "FOREST_GROVE", "DAM", "TUNNEL", "CEMETERY"].includes(
  ...
  if (
    /rest area/i.test(oldName) ||
    /rest stop/i.test(oldName) ||
    /service plaza/i.test(oldName) ||
    hasRestAreaCategory
  ) {
    if (hasRestAreaCategory) {
      if (categories.includes("SCENIC_LOOKOUT_VIEWPOINT")) {
        if (!_wl.restAreaScenic)
          _buttonBanner.restAreaScenic = new Flag.RestAreaScenic();
      }
  ...
  if (region === "SER") {
    if (
      _newCategories.includes("COLLEGE_UNIVERSITY") &&
      _newCategories.includes("PARKING_LOT")
    ) {
      levelToLock = _LOCK_LEVEL_4;
    } else if (
      item.isPoint() &&
      _newCategories.includes("COLLEGE_UNIVERSITY") &&
      (!_newCategories.includes("HOSPITAL_MEDICAL_CARE") ||
  ...
  if (levelToLock > _USER.rank - 1) {
    levelToLock = _USER.rank - 1;
  ...
  if (_buttonBanner.gasNoBrand) {
    if (item.attributes.lockRank >= levelToLock) {
      ...
      _buttonBanner.gasNoBrand.message = `Lock to L${
        levelToLock + 1
      }+ to verify no gas brand.`;
    }
  }
  ...
  if (
    !isLocked &&
    _buttonBanner.extProviderMissing &&
    _buttonBanner.extProviderMissing.active &&
    _buttonBanner.extProviderMissing.severity <= 2
  ) {
    _buttonBanner.extProviderMissing.severity = 3;
    _severityButt = 3;
    if (lockOK) {
      _buttonBanner.extProviderMissing.value = `Lock anyway? (${
        levelToLock + 1
      })`;
      _buttonBanner.extProviderMissing.title =
  ...
  const updatedById = item.attributes.updatedBy
    ? item.attributes.updatedBy
    : item.attributes.createdBy;
  ...
  if (
    ...
    !item.attributes.residential &&
    updatedById &&
    (botRegEx.test(updatedById.toString()) ||
    ...
    if (_USER.isDevUser || _USER.isBetaUser || _USER.rank >= 3) {
      ...
      for (let llix = 1; llix < 6; llix++) {
        if (llix < _USER.rank + 1) {
          if (
            !ddlSelected &&
            (_defaultLockLevel === llix - 1 || llix === _USER.rank)
          ) {
      ...
      _buttonBanner.lockRPP.message = `Current lock: ${
        parseInt(item.attributes.lockRank, 10) + 1
      }. ${_rppLockString} ?`;
    }
  }
  ...
  if (!item.attributes.residential && _severityButt < 3) {
    ...
    if (
      nameShortSpace.includes("'S HOUSE") ||
      nameShortSpace.includes("'S HOME") ||
      nameShortSpace.includes("'S WORK")
    ) {
      if (
        !containsAny(_newCategories, ["RESTAURANT", "DESSERT", "BAR"]) &&
        !pnhNameRegMatch
      ) {
    ...
    if (
      item.attributes.description.toLowerCase().includes("google") ||
      item.attributes.description.toLowerCase().includes("yelp")
    ) {
  ...
  if (hpMode.hlFlag) {
    ...
    Object.keys(_buttonBanner).forEach((tempKey) => {
      if (_buttonBanner[tempKey] && _buttonBanner[tempKey].active) {
        //  If the particular message is active
        if (_buttonBanner[tempKey].hasOwnProperty("WLactive")) {
          if (_buttonBanner[tempKey].WLactive) {
            // If there's a WL option, enable it
            _severityButt = Math.max(
              _buttonBanner[tempKey].severity,
              _severityButt
            );
          }
        } else {
    ...
    // Special case flags
    if (
      item.attributes.lockRank === 0 &&
      (item.attributes.categories.includes("HOSPITAL_MEDICAL_CARE") ||
        item.attributes.categories.includes("HOSPITAL_URGENT_CARE") ||
        item.isGasStation())
    ) {
      _severityButt = 5;
    }
  ...
  _dupeBanner = {};
  if (
    _newName.replace(/[^A-Za-z0-9]/g, "").length > 0 &&
    !item.attributes.residential &&
    !isEmergencyRoom(item) &&
    !isRestArea(item)
  ) {
    if ($("#WMEPH-DisableDFZoom").prop("checked")) {
    ...
    if (_duplicateName.length) {
      if (_duplicateName.length + 1 !== _dupeIDList.length && _USER.isDevUser) {
        ...
        const wlAction = (dID) => {
          ...
          if (!_venueWhitelist[itemID].hasOwnProperty(_wlKeyName)) {
          ...
          _venueWhitelist[itemID].dupeWL = _.uniq(
            _venueWhitelist[itemID].dupeWL
          );
          ...
          if (!_venueWhitelist[dID].hasOwnProperty(_wlKeyName)) {
            // If dupeWL key is not in venue WL, then initialize it.
  ...
  if (_dupeHNRangeList.length > 3) {
    ...
    for (dhnix = 0; dhnix < dupeHNRangeListSorted.length; dhnix++) {
      arrayHNRatio.push(
        Math.abs(
          (parseInt(item.attributes.houseNumber, 10) -
    ...
    if (arrayHNRatio[arrayHNRatioCheckIX] > 1.4) {
      ...
      phlogdev(
        `HN Ratio Score: ${arrayHNRatio[Math.round(arrayHNRatio.length / 2)]}`
      );
    }
  }
...
function assembleBanner() {
  ...
  Object.keys(_buttonBanner).forEach((tempKey) => {
    ...
    if (rowData && rowData.active) {
      ...
      if (rowData.suffixMessage) {
        $rowDiv.append(
          $("<div>").css({ "margin-top": "2px" }).append(rowData.suffixMessage)
        );
      }
  ...
  if (
    _buttonBanner.missingUSPSZipAlt &&
    _buttonBanner.missingUSPSZipAlt.suggestedValue
  ) {
    $("input#WMEPH-zipAltNameAdd").val(
      _buttonBanner.missingUSPSZipAlt.suggestedValue
    );
  }
  ...
  $(".wmeph-pla-spaces-btn").click((evt) => {
    ...
    W.model.actionManager.add(
      new UpdateObject(selectedVenue, {
        categoryAttributes: { PARKING_LOT: newAttr },
      })
    );
  ...
  function resetHoursEntryHeight() {
    ...
    setTimeout(() => {
      ...
      const height =
        lineCount * 18 + 6 + (elem.scrollWidth > elem.clientWidth ? 20 : 0);
  ...
  if (_buttonBanner.noHours) {
    ...
    $("#WMEPH_noHours_2").click(() =>
      _buttonBanner.noHours.replaceHoursAction()
    );
  }
...
function setupButtons(b) {
  Object.keys(b).forEach((tempKey) => {
    // Loop through the banner possibilities
    if (b[tempKey] && b[tempKey].active) {
      //  If the particular message is active
      if (b[tempKey].action && b[tempKey].value) {
        // If there is an action, set onclick
        buttonAction(b, tempKey);
      }
      if (b[tempKey].action2 && b[tempKey].value2) {
...
function showOpenPlaceWebsiteButton() {
  ...
  if (venue) {
    ...
    if (
      openPlaceWebsiteURL &&
      openPlaceWebsiteURL.replace(/[^A-Za-z0-9]/g, "").length > 2
    ) {
      if (!$("#WMEPHurl").length) {
        const strButt1 =
          '<input class="btn btn-success btn-xs wmeph-fat-btn" id="WMEPHurl" title="Open place URL" type="button" value="Website">';
...
function showSearchButton() {
  ...
  if (venue && $("#wmephSearch").length === 0 && !venue.isResidential()) {
    const strButt1 =
      '<input class="btn btn-danger btn-xs wmeph-fat-btn" id="wmephSearch" title="Search the web for this place.  Do not copy info from 3rd party sources!" ' +
  ...
  setTimeout(() => {
    ...
    if (venue) {
      ...
      if ($("#clonePlace").length === 0) {
        let strButt1 =
          '<div style="margin-bottom: 3px;"></div><input class="btn btn-warning btn-xs wmeph-btn" ' +
        ...
        strButt1 =
          '<input class="btn btn-info btn-xs wmeph-btn" id="checkAllClone" title="Check all" ' +
...
function checkHours(hoursObj) {
  ...
  for (let day2Ch = 0; day2Ch < 7; day2Ch++) {
    ...
    const daysObj = [];
    for (let hourSet = 0; hourSet < hoursObj.length; hourSet++) {
      // For each set of hours
      if (hoursObj[hourSet].days.includes(day2Ch)) {
...
function findNearbyDuplicate(
  ...
  let noNumSkip =
    "BANK|ATM|HOTEL|MOTEL|STORE|MARKET|SUPERMARKET|GYM|GAS|GASOLINE|GASSTATION|CAFE|OFFICE|OFFICES" +
  ...
  if (
    ((itemNameNoNum.length > 2 && !noNumSkip.includes(itemNameNoNum)) ||
      allowedTwoLetters.includes(itemNameNoNum)) &&
  ...
  if (selectedVenueAliases.length > 0) {
    for (let aliix = 0; aliix < selectedVenueAliases.length; aliix++) {
      ...
      if (
        (aliasNameRF.length > 2 && !noNumSkip.includes(aliasNameRF)) ||
      ...
      if (
        ((aliasNameNoNum.length > 2 && !noNumSkip.includes(aliasNameNoNum)) ||
        ...
        if (!suppressMatch) {
          ...
          if (
            (strippedTestName.length > 2 &&
              !noNumSkip.includes(strippedTestName)) ||
          ...
          if (
            ((testNameNoNum.length > 2 && !noNumSkip.includes(testNameNoNum)) ||
              allowedTwoLetters.includes(testNameNoNum)) &&
            !testVenueAttr.categories.includes("PARKING_LOT")
          ) {
            ...
            for (let aliix = 0; aliix < testVenueAttr.aliases.length; aliix++) {
              ...
              if (
                ((aliasNameNoNum.length > 2 &&
                  !noNumSkip.includes(aliasNameNoNum)) ||
                  allowedTwoLetters.includes(aliasNameNoNum)) &&
                !testVenueAttr.categories.includes("PARKING_LOT")
              ) {
...
function checkSelection() {
  ...
  if (venue && venue.isApproved()) {
    ...
    for (let dvtix = 0; dvtix < _dupeIDList.length; dvtix++) {
      if (venue.attributes.id === _dupeIDList[dvtix]) {
...
function inferAddress(maxRecursionDepth) {
  ...
  const findConnections = (startingNodeID, recursionDepth) => {
    ...
    for (let idx = 0; idx < keys.length; idx++) {
      ...
      if (hasStreetName(connectedSegments[k].segment)) {
        ...
        newNode =
          attr.fromNodeID === startingNodeID ? attr.toNodeID : attr.fromNodeID;
  ...
  for (i = 0, n = segments.length; i < n; i++) {
    ...
    if (!IGNORE_ROAD_TYPES.includes(segments[i].attributes.roadType)) {
      distanceToSegment = (stopPoint.getPoint
        ? stopPoint.getPoint()
        : stopPoint
      ).distanceTo(segments[i].geometry);
...
function insertAtIX(array1, array2, ix) {
  // array1 is original string, array2 is the inserted string, at index ix
  const arrayNew = array1.slice(); // slice the input array so it doesn't change
  if (typeof array2 === "string") {
    array2 = [array2];
  } // if a single string, convert to an array
  if (typeof array2 === "object") {
    // only apply to inserted arrays
          ...
          () => {
            ...
            $("#PlaceHarmonizerWLToolsMsg")
              ...
              .append(
                $("<p>")
                  .css({ color: "green" })
                  .text(`${venuesToRemove.length} items removed from WL`)
              );
          },
          () => {
            $("#PlaceHarmonizerWLToolsMsg")
...
function onWLShareClick() {
  window.open(
    `https://docs.google.com/forms/d/1k_5RyOq81Fv4IRHzltC34kW3IUbXnQqDVMogwJKFNbE/viewform?entry.1173700072=${_USER.name}`
  );
}
...
function addWmephTab() {
  ...
  const $harmonizerTab = $(
    '<div class="tab-pane active" id="sidepanel-harmonizer"></div>'
  );
  ...
  const $moderatorsTab = $(
    '<div class="tab-pane" id="sidepanel-pnh-moderators"></div>'
  );
  $tabContent.append(
    $harmonizerTab,
    $highlighterTab,
  ...
  $harmonizerTab.append(
    $("<div>").append(
      $("<div>", { style: "font-weight: bold; margin-bottom: 6px;" }).text(
        "Recent updates"
      ),
      Object.keys(_WHATS_NEW_LIST).map((version) =>
        $("<div>").append(
          $("<div>").text(version),
...
function createCloneCheckbox(divID, settingID, textDescription) {
  $(`#${divID}`).append(
    `<input type="checkbox" id="${settingID}">${textDescription}</input>&nbsp&nbsp`
  );
        ...
        if (wlVenue2.hasOwnProperty(wlKey) && wlVenue2[wlKey].active) {
          ...
          if (wlVenue1.hasOwnProperty(wlKey) && wlVenue1[wlKey].active) {
            if (wlVenue1[wlKey].hasOwnProperty("WLKeyArray")) {
              wl1[venueKey][wlKey].WLKeyArray = insertAtIX(
...
function reportError(data) {
  ...
  data.attach_sig = "on";
  if (
    _PM_USER_LIST.hasOwnProperty("WMEPH") &&
    _PM_USER_LIST.WMEPH.approvalActive
  ) {
...
function updateFeatureGeometry(place, newGeometry) {
  ...
  if (
    place &&
    place.CLASS_NAME === "Waze.Feature.Vector.Venue" &&
    newGeometry &&
    (newGeometry instanceof OpenLayers.Geometry.Point ||
      newGeometry instanceof OpenLayers.Geometry.Polygon)
  ) {
    oldGeometry = place.attributes.geometry;
    W.model.actionManager.add(
      new UpdateFeatureGeometry(place, model, oldGeometry, newGeometry)
    );
  }
}
...
function placeHarmonizerInit() {
  ...
  if (
    ...
    if (validateWLS(localStorage.getItem(_WL_LOCAL_STORE_NAME)) === false) {
      ...
      localStorage.setItem(
        "WMEPH-OneTimeWLBU",
        localStorage.getItem(_WL_LOCAL_STORE_NAME)
      );
  ...
  // Settings setup
  if (!localStorage.getItem(_SETTING_IDS.gLinkWarning)) {
    // store settings so the warning is only given once
    localStorage.setItem(_SETTING_IDS.gLinkWarning, "0");
  }
  if (!localStorage.getItem(_SETTING_IDS.sfUrlWarning)) {
    // store settings so the warning is only given once
  ...
  // Event listeners
  W.selectionManager.events.registerPriority("selectionchanged", this, () =>
    errorHandler(checkSelection)
  );
...
function placeHarmonizerBootstrap() {
  if (
    ...
    WazeWrap &&
    WazeWrap.Ready &&
    W.model.categoryBrands.PARKING_LOT &&
    require
  ) {
...
function downloadInternationalSettings(skipBootstrap) {
  ...
  const getSpreadsheetUrl = (id, range, key) =>
    `https://sheets.googleapis.com/v4/spreadsheets/${id}/values/${range}?${dec(
      key
    )}`;

  // TODO change the _PNH_DATA cache to use an object so we don't have to rely on ugly array index lookups.
  const processData1 = (data, colIdx) =>
    data.filter((row) => row.length >= colIdx + 1).map((row) => row[colIdx]);
  ...
  $.getJSON(getSpreadsheetUrl(INT_SHEET_ID, INT_RANGE, API_KEY)).done((res) => {
    ...
    let range = processData1(values, 2);
    let place_url = processData1(values, 3);
    let place_url_addon = processData1(values, 4);
    // now fill the sheets object
    for (var i = countryAbbr.length - 1; i >= 0; i--) {
      // create country data
      _PNH_DATA[countryAbbr[i]] = {
        newPlaceUrl: place_url[i],
        newPlaceAddon: place_url_addon[i],
      };
...
function downloadInternationalPnhData(internat_sheets, skipBootstrap) {
  ...
  for (const [country, settings] of Object.entries(internat_sheets)) {
    phlog("Loading data for " + country);
    // Special case for US and CAN
    const dec = (s) => atob(atob(s));
    const getSpreadsheetUrl = (id, range, key) =>
      `https://sheets.googleapis.com/v4/spreadsheets/${id}/values/${range}?${dec(
        key
    ...
    $.getJSON(getSpreadsheetUrl(settings.id, settings.range, API_KEY))
      .done((res) => {
        const { values } = res;
        ...
        _wordVariations = processData1(values, 11)
          .slice(1)
          .map((row) =>
            row
              .toUpperCase()
        ...
        if (country == "CAN") {
          _PNH_DATA[country].pnh = processData1(values, 2);
          _PNH_DATA[country].pnhNames = makeNameCheckList(
            _PNH_DATA[country].pnh
          );
        } else {
        ...
        _PNH_DATA[country].categories = processData1(values, 1 + coll_offset);
        _PNH_DATA[country].categoryNames = makeCatCheckList(
          _PNH_DATA[country].categories
        );
        ...
        if (country === "USA") {
          const WMEPHuserList = processData1(values, 3 + coll_offset)[1].split(
            "|"
          );
        ...
        const processTermsCell = (termsValues, colIdx) =>
          processData1(termsValues, colIdx)[1]
            .toLowerCase()
            .split("|")
            .map((value) => value.trim());
        _PNH_DATA[country]._hospitalPartMatch = processTermsCell(
          values,
          4 + coll_offset
        );
        _PNH_DATA[country]._hospitalFullMatch = processTermsCell(
          values,
          5 + coll_offset
        );
        _PNH_DATA[country]._animalPartMatch = processTermsCell(
          values,
          6 + coll_offset
        );
        _PNH_DATA[country]._animalFullMatch = processTermsCell(
          values,
          7 + coll_offset
        );
        _PNH_DATA[country]._schoolPartMatch = processTermsCell(
          values,
          8 + coll_offset
        );
        _PNH_DATA[country]._schoolFullMatch = processTermsCell(
          values,
          9 + coll_offset
        );

        phlog("Loaded data for " + country);
      })
      .fail((res) => {
        const message =
...
function savePNHDataToLocalStorage() {
  ...
  let _currentDev = LZString.compressToUTF16(dev_list);
  let _currentBeta = LZString.compressToUTF16(beta_list);
  let _currentPNH = LZString.compressToUTF16(current_loaded);
...
function loadPNHFromLocalStorage() {
  let _localPNH = localStorage.getItem(_PHN_DATA_LOCAL_STORE_NAME);
  if (_localPNH) {
    ...
    try {
      ...
      let _localDev = LZString.decompressFromUTF16(
        localStorage.getItem(_PHN_DATA_LOCAL_STORE_NAME + "devList")
      );
      let _localBeta = LZString.decompressFromUTF16(
        localStorage.getItem(_PHN_DATA_LOCAL_STORE_NAME + "betaList")
      );
...
function bootstrap() {
  ...
  if (unsafeWindow.wmephRunning) {
    WazeWrap.Alerts.error(
      _SCRIPT_NAME,
      "Multiple versions of Place Harmonizer are turned on.  Only one will be enabled."
    );
    return;
  }
...
class HoursParser {
  constructor() {
    ...
    this.DAYS_OF_THE_WEEK = {
      UU: [
        this.translations.day_names[0] + "s",
        this.translations.day_names[0],
        this.translations.abbr_day_names[0],
      ],
      MM: [
        this.translations.day_names[1] + "s",
        this.translations.day_names[1],
        this.translations.abbr_day_names[1],
      ],
      TT: [
        this.translations.day_names[2] + "s",
        this.translations.day_names[2],
        this.translations.abbr_day_names[2],
      ],
      WW: [
        this.translations.day_names[3] + "s",
        this.translations.day_names[3],
        this.translations.abbr_day_names[3],
      ],
      RR: [
        this.translations.day_names[4] + "s",
        this.translations.day_names[4],
        this.translations.abbr_day_names[4],
      ],
      FF: [
        this.translations.day_names[5] + "s",
        this.translations.day_names[5],
        this.translations.abbr_day_names[5],
      ],
      SS: [
        this.translations.day_names[6] + "s",
        this.translations.day_names[6],
        this.translations.abbr_day_names[6],
      ],
    };
    this.MONTHS_OF_THE_YEAR = {
      JAN: [
        this.translations.month_names[1],
        this.translations.abbr_month_names[1],
      ],
      FEB: [
        this.translations.month_names[2],
        this.translations.abbr_month_names[2],
      ],
      MAR: [
        this.translations.month_names[3],
        this.translations.abbr_month_names[3],
      ],
      APR: [
        this.translations.month_names[4],
        this.translations.abbr_month_names[4],
      ],
      MAY: [
        this.translations.month_names[5],
        this.translations.abbr_month_names[5],
      ],
      JUN: [
        this.translations.month_names[6],
        this.translations.abbr_month_names[6],
      ],
      JUL: [
        this.translations.month_names[7],
        this.translations.abbr_month_names[7],
      ],
      AUG: [
        this.translations.month_names[8],
        this.translations.abbr_month_names[8],
      ],
      SEP: [
        this.translations.month_names[9],
        this.translations.abbr_month_names[9],
      ],
      OCT: [
        this.translations.month_names[10],
        this.translations.abbr_month_names[10],
      ],
      NOV: [
        this.translations.month_names[11],
        this.translations.abbr_month_names[11],
      ],
      DEC: [
        this.translations.month_names[12],
        this.translations.abbr_month_names[12],
      ],
    };
