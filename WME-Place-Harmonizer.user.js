...
(function () {
    ...
    const IGNORE_WORDS = "an|and|as|at|by|for|from|hhgregg|in|into|of|on|or|the|to|with".split('|');
    const CAP_WORDS = "3M|AAA|AMC|AOL|AT&T|ATM|BBC|BLT|BMV|BMW|BP|CBS|CCS|CGI|CISCO|CJ|CNN|CVS|DHL|DKNY|DMV|DSW|EMS|ER|ESPN|FCU|FCUK|FDNY|GNC|H&M|HP|HSBC|IBM|IHOP|IKEA|IRS|JBL|JCPenney|KFC|LLC|MBNA|MCA|MCI|NBC|NYPD|PDQ|PNC|TCBY|TNT|TV|UPS|USA|USPS|VW|XYZ|ZZZ".split('|');
    const SPECIAL_WORDS = "d'Bronx|iFix".split('|');
    ...
    class App {
        ...
        toTitleCase(str) {
            ...
            // lowercase any from the IGNORE_WORDS list
            str = str.replace(/[^ ]+/g, function(txt) {
                let txtLC = txt.toLowerCase();
                return (IGNORE_WORDS.indexOf(txtLC) > -1) ? txtLC : txt;
            });
            // uppercase any from the CAP_WORDS List
            str = str.replace(/[^ ]+/g, function(txt) {
                let txtUC = txt.toUpperCase();
                return (CAP_WORDS.indexOf(txtUC) > -1) ? txtUC : txt;
            });
            // preserve any specific words
            str = str.replace(/[^ ]+/g, function(txt) {
                //let txtAC = txt.toUpperCase();
                for (let swix=0; swix<SPECIAL_WORDS.length; swix++) {
                    if ( txt.toUpperCase() === SPECIAL_WORDS[swix].toUpperCase() ) {
                        return SPECIAL_WORDS[swix];
                    }
                }
                return txt;
            });
        ...
        toTitleCaseStrong(str) {
            ...
            str = str.replace(/[^ ]+/g, function(txt) {
                ...
                return (CAP_WORDS.indexOf(txtLC) > -1) ? txtLC : txt;
            });
        ...
        setServiceChecked(servBtn, checked, actions) {
            ...
            if (checkboxChecked === servBtn.checked && checkboxChecked !== checked) {
                ...
                if (!noAdd) {
                    this.addUpdateAction({services:services}, actions);
        ...
        makeNameCheckList(PNH_DATA) {  // Builds the list of search names to match to the WME place name
            ...
            for (let pnhix=0; pnhix<PNH_DATA.length; pnhix++) {  // loop through all PNH places
                ...
                if (pnhEntryTemp[ph_disable_ix] !== "1" || pnhEntryTemp[ph_speccase_ix].indexOf('betaEnable') > -1 ) {
                    ...
                    newNameList = this.uniq(newNameList);  // remove any duplicate search names
        ...
        deleteDupeLabel(){
            ...
            setTimeout(function() {
                ...
                if ( lastAction && lastAction.hasOwnProperty('object') && lastAction.object.hasOwnProperty('state') && lastAction.object.state === 'Delete' ) {
        ...
        static checkDataReady(callCount) {
            ...
            if ("undefined" !== typeof CAN_PNH_DATA && "undefined" !== typeof USA_PNH_DATA && "undefined" !== typeof USA_CH_DATA &&
                ...
                if (callCount<200) {
                    callCount++;
                    setTimeout(() => this.checkDataReady(callCount), 100);
                } else {
        ...
        static addPURWebSearchButton() {
            ...
            function panelContainerChanged() {
                if (!$('#WMEPH-HidePURWebSearch' + DEV_VERS_STR).is(':checked')) {
                    let $panelNav = $('.place-update-edit.panel .categories.small');
                    if ($('#PHPURWebSearchButton').length === 0 && $panelNav.length > 0) {
                        let $btn = $('<button>', {class:"btn btn-primary", id:"PHPURWebSearchButton"}) //NOTE: Don't use btn-block class. Causes conflict with URO+ "Done" button.
    ...
    class Venue {
        ...
        get brand() {return this.attributes.brand;}
        get categories() {return this.attributes.categories;}
        get openingHours() {return this.attributes.openingHours;}
        get lockRank() {return this.attributes.lockRank;}
        get geometry() {return this.attributes.geometry;}

        getLonLat() {
            return OpenLayers.Layer.SphericalMercator.inverseMercator(this.geometry.getCentroid().x,this.geometry.getCentroid().y);
        }
        plaOperatorIsValid() {
            return this.isPLA() && W.model.venues.categoryBrands.PARKING_LOT.indexOf(this.brand) > -1;
        }
        isPLA() {
            return this.categories && this.categories.indexOf('PARKING_LOT') > -1;
        }
        isGasStation() {
            return this.categories && this.categories.indexOf('GAS_STATION') > -1;
        }
        isAlwaysOpen() {
            let first = this.attributes.openingHours[0];
            return !!first && first.days.length === 7 && first.fromHour === '00:00' && first.toHour === '00:00';
        }
        isEmergencyRoom() {
            return /(?:emergency\s+(?:room|department|dept))|\b(?:er|ed)\b/i.test(this.name);
        }
        getPvaSeverity(pvaValue) {
            let isER = pvaValue === 'hosp' && this.isEmergencyRoom();
            return (pvaValue === '' || pvaValue === '0' || (pvaValue === 'hosp' && !isER)) ? 3 : (pvaValue === '2') ? 1 : (pvaValue === '3') ? 2 : 0;
        }

        // Formats "hour object" into a string.
        static formatOpeningHour(hourEntry) {
        ...
        openingHoursToNaturalText() {
            return this.openingHours && this.openingHours.map(Venue.formatOpeningHour);
        }

        // Parse hours paste for hours object array
        static parseHours(inputHours) {
            let daysOfTheWeek = {
            ...
            inputHoursParse = inputHoursParse
                ...
                .replace(/closed/g, '99:99-99:99').replace(/not open/g, '99:99-99:99')  // parse 'closed'
                .replace(/by appointment only/g, '99:99-99:99').replace(/by appointment/g, '99:99-99:99')  // parse 'appointment only'
                .replace(/weekdays/g, 'mon-fri').replace(/weekends/g, 'sat-sun')  // convert weekdays and weekends to days
                .replace(/12:00 noon/g, "12:00").replace(/12:00 midnight/g, "00:00")  // replace 'noon', 'midnight'
                .replace(/12 noon/g, "12:00").replace(/12 midnight/g, "00:00")  // replace 'noon', 'midnight'
                .replace(/noon/g, "12:00").replace(/midnight/g, "00:00")  // replace 'noon', 'midnight'
                .replace(/every day/g, "mon-sun")  // replace 'seven days a week'
                .replace(/seven days a week/g, "mon-sun")  // replace 'seven days a week'
                .replace(/7 days a week/g, "mon-sun")  // replace 'seven days a week'
                .replace(/daily/g, "mon-sun")  // replace 'open daily'
                .replace(/open 24 ?ho?u?rs?/g, "00:00-00:00")  // replace 'open 24 hour or similar'
                .replace(/open twenty\-? ?four ho?u?rs?/g, "00:00-00:00")  // replace 'open 24 hour or similar'
                .replace(/24 ?ho?u?rs?/g, "00:00-00:00")  // replace 'open 24 hour or similar'
                .replace(/twenty\-? ?four ho?u?rs?/g, "00:00-00:00")  // replace 'open 24 hour or similar'
            ...
            if ( Venue.checkOverlappingHours(hoursObjectArraySorted) ) {
        ...
        hasOverlappingHours() {
            return Venue.checkOverlappingHours(this.openingHours);
        }
        
        // function to check overlapping hours
        static checkOverlappingHours(hoursObj) {
    ...
    function runPH() {
        ...
        function normalizePhone(s, outputFormat, returnType) {
            if ( !s && returnType === 'existing' ) {
                if (!currentVenue.isPLA() || (currentVenue.brand && currentVenue.plaOperatorIsValid())) {
        ...
        function harmonizePlaceGo(item, useFlag, actions) {
            ...
            if (hpMode.hlFlag && currentVenue.isPLA() && currentVenue.lockRank === 0) {
                return 4;
            }

            //  set up external post div and pull place PL
            let placePL = getItemPL()
            ...
            bannButt = {
                ...
                phoneMissing: {
                    ...
                    action: function() {
                        ...
                        let newPhone = normalizePhone(newPhoneVal, this.outputFormat, 'inputted');
                ...
                plaHasAccessibleParking: {
                    ...
                    action: function() {
                        let services = item.attributes.services;
                        if (services) {
                ...
                noHours: {
                    ...
                    action: function() {
                        ...
                        pasteHours = pasteHours + ',' + currentVenue.openingHours.join(',');
                        let hoursObjectArray = Venue.parseHours(pasteHours);
            ...
            if (v.isPLA()) {
                ...
                if (parkAttr && !parkAttr.canExitWhileClosed && ($('#WMEPH-ShowPLAExitWhileClosed' + DEV_VERS_STR).prop('checked') || !(currentVenue.isAlwaysOpen() || currentVenue.openingHours.length === 0))) {
            ...
            if ( venueWhitelist.hasOwnProperty(itemID) ) {
                if ( hpMode.harmFlag || ( hpMode.hlFlag && !$("#WMEPH-DisableWLHL" + DEV_VERS_STR).prop('checked')  ) ) {
                    ...
                    venueWhitelist[itemID].gps = currentVenue.getLonLat();  // Store GPS coords for the venue
                }
            }
            ...
            if (item.attributes.residential) {
            ...
            } else if (currentVenue.isPLA() || (newName && newName.trim().length > 0)) {  // for non-residential places
                if (usrRank >= 3 && !(currentVenue.isPLA() && $('#WMEPH-DisablePLAExtProviderCheck' + DEV_VERS_STR).prop('checked'))) {
                ...
                if (PNHMatchData[0] !== "NoMatch" && PNHMatchData[0] !== "ApprovalNeeded" && PNHMatchData[0] !== "Highlight" ) { // *** Replace place data with PNH data
                    ...
                    if (ph_sfurl_ix > -1) {  // if the sfurl column exists...
                        if ( ph_sfurllocal_ix > -1 && PNHMatchData[ph_sfurllocal_ix] !== "" && PNHMatchData[ph_sfurllocal_ix] !== "0" ) {
                            ...
                            let lonLat = currentVenue.getLonLat();
                            for (let tlix = 1; tlix<tempLocalURL.length; tlix++) {
                                if (tempLocalURL[tlix] === 'ph_streetName') {
                                    ...
                                    customStoreFinderLocalURL = customStoreFinderLocalURL + lonLat.lat;
                                } else if (tempLocalURL[tlix] === 'ph_longitudePM') {
                                    customStoreFinderLocalURL = customStoreFinderLocalURL + lonLat.lon;
                ...
                for(let ixPlaceCat=0; ixPlaceCat<newCategories.length; ixPlaceCat++) {
                    ...
                    if (ixPNHCat>-1) {
                        ...
                        let pointSeverity = currentVenue.getPvaSeverity(pvaPoint);
                        let areaSeverity = currentVenue.getPvaSeverity(pvaArea);
                ...
                if ( currentVenue.hasOverlappingHours() ) {
                    //phlogdev('Overlapping hours');
            ...
            dupeHNRangeList = [];
            bannDupl = {};
            if (newName.replace(/[^A-Za-z0-9]/g,'').length > 0 && !item.attributes.residential && !currentVenue.isEmergencyRoom()) {
        ...
        function findNearbyDuplicate(itemName, itemAliases, item, recenterOption) {
            ...
            for (let venix in venueList) {  // for each place on the map:
                if (venueList.hasOwnProperty(venix)) {  // hOP filter
                    ...
                    let otherVenue = new Venue(venueList[venix]);
                    if ((!excludePLADupes || (excludePLADupes && !(currentVenue.isPLA() || otherVenue.isPLA()))) && !otherVenue.isEmergencyRoom()) {
                        let pt2ptDistance =  currentVenue.geometry.getCentroid().distanceTo(otherVenue.geometry.getCentroid());
        ...
        function add_PlaceHarmonizationSettingsTab() {
            ...
            createSettingsCheckbox("sidepanel-harmonizer" + DEV_VERS_STR, "WMEPH-HidePURWebSearch" + DEV_VERS_STR,"Hide 'Web Search' button on PUR popups");
